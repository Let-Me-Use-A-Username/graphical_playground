...
use macroquad::{color::*, math::Vec2, time::get_time};
...
impl WaveConfig{
    fn new(spawn_interval: f64) -> WaveConfig{
        let complexity = EnemyComplexity::Simple;

        return WaveConfig{
            enemy_count: (complexity as u64 * 10),
...
impl SpawnManager{
    pub fn new(sender: Sender<Event>, level_interval: f64, spawn_interval: f64) -> SpawnManager{
        return SpawnManager{
            ...
            config: WaveConfig::new(spawn_interval),
            sender: sender
        }
    }

    pub async fn update(&mut self, player_pos: Vec2, active_enemies: usize){
        let now = get_time();
        ...
        if self.spawn_timer.expired(now){
            self.spawn_timer.set(now, self.config.spawn_interval);
            let template = self.get_spawn_template();
        ...
        //If enemies active are less than the `enemy_count`, spawn the differense
    ...
    fn advance_level(&mut self, now: f64){
        ...
        self.config.complexity = self.config.complexity.next();
        self.config.enemy_count = self.config.complexity as u64 * 10
    }

    #[inline(always)]
    fn get_spawn_template(&self) -> VecDeque<EnemyType>{
        let mut count = self.config.enemy_count.clone();
        let complexity = self.config.complexity;

        let mut template: VecDeque<EnemyType> = VecDeque::with_capacity(count as usize);
